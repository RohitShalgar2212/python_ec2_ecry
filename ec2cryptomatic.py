    ...
    def _stop_instance(self):
        try:
           if self._instance.state['Name'] == 'running':
              self._logger.info('-> Stoping instance %s' % self._instance.id)
              self._ec2_client.stop_instances(InstanceIds=[self._instance.id])
              self._instance.wait_until_stopped()
        except: 
              self._logger.info('-->instance is already stop'  % self._instance.id)
